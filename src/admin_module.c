...
int modify_user(uint32_t userId, const char *name, int age, const char *address, char *resp_msg, size_t resp_sz) {
    modify_user_data data = {name, age, address, resp_msg, resp_sz};
...
int change_role_modifier(user_rec_t *user, void *data) {
    ...
    role_t new_role;
    if (strcmp(d->role_str, "customer") == 0) new_role = ROLE_CUSTOMER;
    else if (strcmp(d->role_str, "employee") == 0) new_role = ROLE_EMPLOYEE;
    else if (strcmp(d->role_str, "manager") == 0) new_role = ROLE_MANAGER;
    else if (strcmp(d->role_str, "admin") == 0) new_role = ROLE_ADMIN;
    else {
